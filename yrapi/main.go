...
func WeatherIcon(symbol int, contentType string, isNight bool, isPolarNight bool) string {
	...
	url := fmt.Sprintf(
		"http://api.met.no/weatherapi/weathericon/1.1/?symbol=%d;is_night=%d;is_polarnight=%d;content_type=%s",
...
func LocationforecastLTS(lat float64, lng float64) (WeatherData, error) {
	...
	url := fmt.Sprintf("http://api.met.no/weatherapi/locationforecastlts/1.3/?lat=%3.5f;lon=%3.5f", lat, lng)
